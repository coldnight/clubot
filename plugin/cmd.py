...
class AdminCMDHandle(CommandHandler):
    ...
    def rm(self, stanza, *args):
        ...
        if emails >= 1:
            ...
            for e in emails:
                jid = JID(e)
                p.append(Presence(from_jid = stanza.to_jid, to_jid = jid,stanza_type='unsubscribe'))
                p.append(Presence(from_jid = stanza.to_jid, to_jid = jid, stanza_type='unsubscribed'))
                del_member(jid)
        else:
...
def send_all_msg(stanza, body, system=False):
    """
    发送所有消息
    - `stanza` : 来源消息结
    - `body`   : 发送消息主体
